...
public interface AppointmentRepository extends JpaRepository<Appointment, Long> {
    ...
    @Query(value = "SELECT * FROM appointment WHERE doctor = ?1 AND date = ?2", nativeQuery = true)
	public Appointment findByDoctorAndDate(Long id, Date date);

    @Query(value = "SELECT * FROM appointment WHERE patient = ?1 AND date = ?2", nativeQuery = true)
